...
[제목 없음](https://www.notion.so/5b52bd6545244d6ebe48cf321d297deb)
...
function solution(N, stages) {
  ...
  for (let i = 1; i <= N; i++) {
    answer.push(i);
    const arrivedPlayer = stages.filter((stage) => stage >= i);
    const clearedYet = arrivedPlayer.filter((stage) => stage === i);
    if (!arrivedPlayer.length) failureRate[i] = 0;
    else failureRate[i] = clearedYet.length / arrivedPlayer.length;
  }

  answer.sort((a, b) => failureRate[b] - failureRate[a]);
